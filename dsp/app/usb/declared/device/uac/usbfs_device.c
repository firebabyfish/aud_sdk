...
__attribute__((aligned(4))) uint8_t USBFS_EP2_Buf[DEF_USBD_EP2_FS_SIZE];
...
{
    USBFSD->UEP2_3_MOD = USBFS_UEP2_TX_EN | USBFS_UEP2_RX_EN | USBFS_UEP3_TX_EN | USBFS_UEP3_RX_EN;
    ...
    USBFSD->UEP2_TX_LEN  = DEF_USBD_EP2_FS_SIZE;
...
static void app_usb_uac_setup_handler(uint8_t ep_idx)
{
    uint8_t errflag = 0;
    uint16_t len    = 0;
        ...
        if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS) {
            if ((USBFS_SetupReqType & USB_UAC_REQ_TYPE_MASK) == USB_UAC_REQ_TYPE_ID_INF) {
                switch (USBFS_SetupReqCode) {
                    case UAC_GET_CUR:
                        if (USBFS_SetupReqIndex == 0x0200) { /* Entity ID: 2 */
                        ...
                        } else if (USBFS_SetupReqIndex == 0x0500) { /* Entity ID: 5 */
                            switch ((USBFS_SetupReqValue >> 8) & 0xFF) {
                                case UAC_CS_MUTE_CONTROL:
            ...
            case USB_GET_DESCRIPTOR:
                switch ((uint8_t)(USBFS_SetupReqValue >> 8)) {
                    ...
                    case USB_DESCR_TYP_DEVICE:
                        pUSBFS_Descr = uac_dev_descr;
                        len          = DEF_USBD_DEVICE_DESC_LEN;
                        break;

                    /* get usb configuration descriptor */
                    case USB_DESCR_TYP_CONFIG:
                        pUSBFS_Descr = uac_cfg_descr;
                    ...
                    case USB_DESCR_TYP_STRING:
                        switch ((uint8_t)(USBFS_SetupReqValue & 0xFF)) {
                            ...
                            case DEF_STRING_DESC_LANG:
                                pUSBFS_Descr = uac_lang_descr;
                                len          = DEF_USBD_LANG_DESC_LEN;
                                break;

                            /* Descriptor 1, Manufacturers String descriptor */
                            case DEF_STRING_DESC_MANU:
                                pUSBFS_Descr = uac_manu_info;
                                len          = DEF_USBD_MANU_DESC_LEN;
                                break;

                            /* Descriptor 2, Product String descriptor */
                            case DEF_STRING_DESC_PROD:
                                pUSBFS_Descr = uac_prod_info;
                                len          = DEF_USBD_PROD_DESC_LEN;
                                break;

                            /* Descriptor 3, Serial-number String descriptor */
                            case DEF_STRING_DESC_SERN:
                                pUSBFS_Descr = uac_serial_info;
...
static void app_usb_uac_in_handler(uint8_t ep_idx)
{
    uint16_t len = 0;

    switch (ep_idx) {
        case DEF_UEP0: { /* end-point 0 data in interrupt */
            if (USBFS_SetupReqLen == 0) {
                USBFSD->UEP0_RX_CTRL = USBFS_UEP_R_TOG | USBFS_UEP_R_RES_ACK;
            }
            if ((USBFS_SetupReqType & USB_REQ_TYP_MASK) != USB_REQ_TYP_STANDARD) {
        ...
        case DEF_UEP3: /* end-point 3 data in interrupt */
...
static void app_usb_uac_out_handler(uint8_t ep_idx)
{
    uint8_t errflag = 0;
    uint16_t len    = 0;
...
static void app_usb_uac_sof_handler(uint8_t ep_idx)
{
}

static void app_usb_uac_irq_handler(void)
{
    ...
    if (intflag & USBFS_UIF_TRANSFER) {
        ...
        switch (intst & USBFS_UIS_TOKEN_MASK) {
            case USBFS_UIS_TOKEN_IN:
                app_usb_uac_in_handler(intst & USBFS_UIS_ENDP_MASK);
                break;
            /* data-out stage processing */
            case USBFS_UIS_TOKEN_OUT:
                app_usb_uac_out_handler(intst & USBFS_UIS_ENDP_MASK);
                break;
            /* Setup stage processing */
            case USBFS_UIS_TOKEN_SETUP:
                app_usb_uac_setup_handler(intst & USBFS_UIS_ENDP_MASK);
                break;
            /* Sof pack processing */
            case USBFS_UIS_TOKEN_SOF:
                app_usb_uac_sof_handler(intst & USBFS_UIS_ENDP_MASK);
                break;
