...
#define AUDIO_IN_PACKET  ((uint32_t)((AUDIO_MIC_FREQ * AUDIO_MIC_FRAME_SIZE_BYTE * 1) / 1000))
...
#endif
    AUDIO_AS_DESCRIPTOR_INIT(0x02, 0x03, 0x01, AUDIO_MIC_FRAME_SIZE_BYTE, AUDIO_MIC_RESOLUTION_BIT, AUDIO_IN_EP, 0x05, AUDIO_IN_PACKET,
...
{
    // APP_LOG_INFO("actual out len:%d", (unsigned int)nbytes);
    usbd_ep_start_read(busid, AUDIO_OUT_EP, read_buffer, AUDIO_OUT_PACKET);
}
...
void app_usb_audio_runloop(uint8_t busid)
{
    if (tx_flag) {
        ...
        usbd_ep_start_write(busid, AUDIO_IN_EP, test_48k_16bits_sin_tone_1Khz, AUDIO_IN_PACKET);
